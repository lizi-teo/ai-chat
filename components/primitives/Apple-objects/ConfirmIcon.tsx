...
export function ConfirmIcon({ className }: { className?: string }) {
  const id = useId()
  const clipId = `confirm-clip-${id}`
  const maskId = `confirm-mask-${id}`
  ...
  return (
    <svg
      ...
      fill="none"
      aria-hidden
      className={cn('text-apple-pay-blue', className)}
    >
      <g clipPath={`url(#${clipId})`}>
        <rect x="-2.335" y="5.665" width="19.67" height="39.67" rx="3.335" fill="currentColor" fillOpacity="0.15" />
        <rect x="-2.335" y="5.665" width="19.67" height="39.67" rx="3.335" stroke="currentColor" strokeWidth="1.33" />
        <rect x="1.5" y="8.5" width="8" height="1" rx="0.5" stroke="currentColor" />
        <mask id={maskId} fill="white">
          <path d="M18 13.67V13.67C18.5523 13.67 19 14.1177 19 14.67V20.336C19 20.8883 18.5523 21.336 18 21.336V21.336V13.67Z" />
        </mask>
        <path
